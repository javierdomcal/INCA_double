...
intracule_zero=0.d0
 do while (.true.)  !loop for primitive quartets.
   ...
   call CPU_time(TT1)
   read(5,iostat=kk1) i, j, k, l, DMval
   call CPU_time(TT2)
   Tread=Tread+(TT2-TT1)
   if (kk1 .lt. 0) go to 100 !end of file
 
   ! Primitive normalization factors  
   if (normalize_dm2p) then
        n_prim_i=((2.d0*alfa(i)/pi)**(3.d0/4.d0))*(((4.d0*alfa(i))**(Ltot(i)))/dfact(2*Ltot(i)+1))
        n_prim_j=((2.d0*alfa(j)/pi)**(3.d0/4.d0))*(((4.d0*alfa(j))**(Ltot(j)))/dfact(2*Ltot(j)+1))
        n_prim_k=((2.d0*alfa(k)/pi)**(3.d0/4.d0))*(((4.d0*alfa(k))**(Ltot(k)))/dfact(2*Ltot(k)+1))
        n_prim_l=((2.d0*alfa(l)/pi)**(3.d0/4.d0))*(((4.d0*alfa(l))**(Ltot(l)))/dfact(2*Ltot(l)+1))
        n_prim_t=n_prim_i*n_prim_j*n_prim_k*n_prim_l !total normalization
   else
        n_prim_t=1.d0
   end if
   
   ! Calculate distances
   R_i_k_2=(cartes(Ra(i),1)-cartes(Ra(k),1))**2.d0+(cartes(Ra(i),2)-cartes(Ra(k),2))**2.d0+(cartes(Ra(i),3)-cartes(Ra(k),3))**2.d0
   R_j_l_2=(cartes(Ra(j),1)-cartes(Ra(l),1))**2.d0+(cartes(Ra(j),2)-cartes(Ra(l),2))**2.d0+(cartes(Ra(j),3)-cartes(Ra(l),3))**2.d0
   
   ! Store DM2 for accuracy check
   trDM2=trDM2+DMval
   trace_DM2prim=trace_DM2prim+DMval*n_prim_t
   
   ! Calculate grid-independent part
   call CPU_time(TT3)
   call variables_ijkl(i,j,k,l)  !set up variables for the ijkl quartet
   A_ind=(a_ijkl)**(-1.5d0)*dexp(-e_ik*R_i_k_2-e_jl*R_j_l_2) !eq.18
   
   ! First integral screening
   screen1=DMval*n_prim_t*A_ind
   call CPU_time(TT4)
   T1screen=T1screen+(TT4-TT3)
   
   if (dabs(screen1).gt.lim) then    !1st screening test
       
       ! Calculate polynomial degrees and coefficients
       sm=0
       U_x=0.d0; U_y=0.d0; U_z=0.d0
       do ii=1,3
             Lrtot(ii)=TMN(i,ii)+TMN(j,ii)+TMN(k,ii)+TMN(l,ii) !Lrtot=degree of eqn 15
             ...
             !compute U coefficients for second integral screening
             do iii=1,np
       ...
       ! Second integral screening
       screen2=dabs(DMval*n_prim_t*A_ind*U_x*U_y*U_z) !eqn.40
       call CPU_time(TT4)
       T2screen=T2screen+(TT4-TT3)                
       
       if (screen2.ge.lim) then    !2nd screening test
                 Aa_ijkl=DMval*n_prim_t*A_ind    !grid independent part 
                 ...
                 do ig=1,ngrid                                             
                            ...
                            do ii=1,3                                       !loop for x y and z  
                                 ...
                                 do iii=1,Lrtot(ii)+1   !Ltot+1 is the number of coefficients
                            ...
                            !Calculate intracule at this point
                            I_vec(ig)=I_vec(ig)+Aa_ijkl*dexp(-(rp(1)**2.d0+rp(2)**2.d0+rp(3)**2.d0))*V_x*V_y*V_z
                                                                     
                 end do       !End loop over grid points
                 
                 !compute intracule at 0,0,0 for intracule_zero
                 V_x=0.d0; V_y=0.d0; V_z=0.d0
                 do ii=1,3
                     rp(ii)=sqe*(0.d0+r_ik(ii)-r_jl(ii))   !compute R'(eq. 19), at 0,0,0
                     pot=dble(Lrtot(ii))
...
if (vee_flag) then        
      ...
      ! Output all Vee results through unified system - no more write statements!
